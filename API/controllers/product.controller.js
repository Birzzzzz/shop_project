...
exports.createProduct = (req,result) => {
    ...
    db.query("INSERT INTO products SET ?",product,(error,res) =>{
        if(error){
            ...
            console.log(date + " : Une erreur est survenue lors de l'envoie de la requête (api/product/addProduct).");
            console.log(error);
            return;
        }
        ...
        console.log(date + " : Le produit avec le nom " + req.body.productName + " a bien été ajouter.");
        return;
    });
};
...
exports.updateById = (req,result) => {
    ...
    db.query(`UPDATE products SET productName = '${req.body.productName}, price = ${req.body.price}, description = '${req.body.description}'
    ,companyId = ${req.body.companyId}, categoryId = ${req.body.categoryId} WHERE id = ${req.params.id}`, (error,res) => {
        if(error){
            result.status(500).send({
                message:"Une erreur est survenue lors de l'envoie de la requête."
            });
            console.log(date + " : Une erreur est survenue lors de l'envoie de la requête (api/products/update/:id).");
            console.log(error);
            return;
        }
